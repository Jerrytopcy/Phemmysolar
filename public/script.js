...
async function proceedToCheckout() {
  ...
  try {
    ...
    // 👇 SEND ORDER TO BACKEND INSTEAD OF SAVING LOCALLY
...
async function loadOrderHistory() {
  ...
  try {
    ...
    // Sort orders by date (newest first) if not already sorted by backend
    ...
    container.innerHTML = '<p>Error loading order history. Please try again.</p>';
  }
}
...
// ⛔ REMOVED: checkPaymentStatus and updateOrderPaymentStatus functions — they mutate local orders
// These functions are now obsolete since we're using backend APIs for order management.
